...
public class Point implements Comparable<Point> {
	...
	public double slopeTo(Point that) {
		...
		if (this.x == that.x && this.y == that.y) {
			...
			returnValue = (double) (that.y - this.y) / (double) (that.x - this.x);
		}
		return returnValue;
	}
	...
	private class SlopeOrderComparator implements Comparator<Point> {
		...
		public int compare(Point thisPoint, Point thatPoint) {
			double slopeResult1 = slopeTo(thisPoint);
			double slopeResult2 = slopeTo(thatPoint);

			if ((slopeResult1 == Double.POSITIVE_INFINITY && slopeResult2 == Double.POSITIVE_INFINITY)
					|| (slopeResult1 == Double.NEGATIVE_INFINITY && slopeResult2 == Double.NEGATIVE_INFINITY)) {
				return 0;
			}

			if (Math.abs(slopeResult1 - slopeResult2) < epsilon) {
				return 0;
			} else if (slopeResult1 < slopeResult2) {
				return -1;
			} else {
				return 1;
			}
